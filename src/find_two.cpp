...
FindTwo::FindTwo()
{
	...
	pos_pub = node.advertise<geometry_msgs::Point>("ardrone_pos", 1000);
}
...
void FindTwo::image_process(Mat img)
{
	...
	distant = get_distant();
	angle = acos(project(level)/LEVEL);
...
void FindTwo::find_two_red(Mat &img, Mat imgThresholded)
{
	...
	level = fabs(0.5*(rb[0].tl().x + rb[0].br().x - rb[1].tl().x - rb[1].br().x));
}
...
double FindTwo::get_distant()
{

}

double FindTwo::project(double l_img)
{
